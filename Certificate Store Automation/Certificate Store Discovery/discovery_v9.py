...
def check_certstore_server(env, registration, server_node, discovery_type):
    if discovery_type == 'windows':
        server_node = f'http://{server_node}:5985'
    urlpath = f"KeyfactorApi/CertificateStores/Server?pq.queryString=Name%20-eq%20%22{server_node}%22%20AND%20ServerType%20-eq%20{registration}"
    fullurl = f'{KEYFACTOR_URLS[env]}{urlpath}'
    f = requests.get(fullurl, auth=(KEYFACTOR_ID[env], KEYFACTOR_PASS[env]), proxies=PROXY, headers=HEADERS,
                     verify=True)
    if not ok_codes(f):
...
def create_cert_store(env, server_node, registration, discovery_type, username, password):
    if discovery_type == 'windows':
        server_node = f'http://{server_node}:5985'
    URL = f"KeyfactorApi/CertificateStores/Server"
    fullURL = f'{KEYFACTOR_URLS[env]}{URL}'
    define_body: dict = {
        "UseSSL": False,
        "ServerType": registration,
        "Name": server_node,
        'Username': {'SecretValue': username},
        'Password': {'SecretValue': password}
    }
    define_body = json.dumps(define_body)
    f = requests.post(fullURL, auth=(KEYFACTOR_ID[env], KEYFACTOR_PASS[env]), data=define_body, proxies=PROXY, headers=HEADERS, verify=True)
    if not ok_codes(f):
        print(f'Received a {f.status_code} attempting a post on {f.url} received message {f.content}')
        sys.exit(16)
    return
...
def schedule_discovery(env, server_node, orchestrator_id, discovery_type, exectime, username, password):
    ...
    for stype in stypes:
        if stype['name'] == 'JKS' or stype['name'] == 'PEM':
            continue
        registration = stype['serverregistration']
        certstore_server = check_certstore_server(env, registration, server_node, discovery_type)
        if len(certstore_server) > 0:
            create_cert_store(env, server_node, stype, discovery_type,username, password)
        name = stype['name']
        ...
        body['JobExecutionTimestamp'] = exectime
...
def main():
    ...
    parser.add_argument("-s", "--server_node", help="Fully Qualified Name of the server or node to run the certificate discovery on")
    parser.add_argument("-r", "--run_every_time", help="time the job is effective by value must be in format (HH:MM:SS)")
    parser.add_argument("-ef", "--effective_date", help="date the job is effective by value must be in format (YYYY-mm-dd)")
    parser.add_argument("-d", "--dayofweek", help="day this discovery job will run, values are (mon, tue, wed, thu, fri, sat, sun)")
