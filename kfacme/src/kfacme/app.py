...
def extract_sub_from_token(token):
    ...
    decoded_token = jwt.decode(
        ...
        key=key,
        algorithms=["RS256"],
        audience=variables.get("audience"),  # Optional audience variable
...
def display_templates_and_get_choice(templates):
    ...
    template_choice = input("Enter the ID of the Keyfactor Enrollment Pattern to associate with this claim or press Enter to return: ").strip()
...
def user():
    ...
    while True:
        ...
        if choice == 1:
            ...
            click.secho("Return to main menu", fg="cyan")
            main_menu()
        else:
            click.secho("Invalid choice. Please enter a number between 1 and 4.", fg="red")
            click.pause()
...
@click.command()
def main_menu(base_path: str = ".") -> int:
    ...
    REQUIRED = ["client_id", "scope", "audience", "oidc_discovery_url","acme_dns", "keyfactor_dns","cert_check"]
    ...
    #start Main Menu
    """Present the Main Menu and handle user choices."""
    click.secho("Welcome to the Keyfactor ACME Claims Management Tool!", fg="bright_green")
    while True:
        click.clear()
        click.secho("=== Main Menu ===", fg="cyan")
        click.echo("[1] Administration")
        click.echo("[2] Account Admin")
        click.echo("[3] Enrollment User")
        click.echo("[4] Exit")

        choice = click.prompt("Choose what account type you have (1-4)", type=int)
